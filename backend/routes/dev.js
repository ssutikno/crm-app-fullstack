...
// This endpoint should be protected or removed in a real production environment
router.post('/seed', async (req, res) => {
    try {
        ...
        try {
            ...
            await client.query('TRUNCATE users, customers, contacts, leads, deals, tasks, product_attachments, quotes, quote_line_items, product_requests RESTART IDENTITY CASCADE');
            console.log("Cleared existing sample data tables.");

            // 1. Seed Users with freshly hashed passwords
            console.log("Seeding users...");
            const usersToInsert = [
                { id: 1, name: 'John Doe', email: 'john.doe@example.com', role: 'sales', status: 'Active'},
                { id: 2, name: 'Jane Smith', email: 'jane.smith@example.com', role: 'sales_manager', status: 'Active'},
                { id: 3, name: 'Mike Ross', email: 'mike.ross@example.com', role: 'sales', status: 'Active'},
                { id: 4, name: 'Peter Pan', email: 'peter.pan@example.com', role: 'product_manager', status: 'Active'},
                { id: 5, name: 'Susan Storm', email: 'susan.storm@example.com', role: 'supervisor', status: 'Active'},
                { id: 6, name: 'Ben Grimm', email: 'ben.grimm@example.com', role: 'telesales', status: 'Inactive'},
                { id: 99, name: 'Default Admin', email: 'admin@example.com', role: 'supervisor', status: 'Active'}
            ];
            
            const salt = await bcrypt.genSalt(10);
            const plainTextPassword = 'password123'; // Use one password for all samples
            const hashedPassword = await bcrypt.hash(plainTextPassword, salt);
            
            const userQuery = 'INSERT INTO users (id, name, email, password_hash, role_id, status) VALUES ($1, $2, $3, $4, $5, $6)';
            for(const user of usersToInsert) {
                await client.query(userQuery, [user.id, user.name, user.email, hashedPassword, user.role, user.status]);
            }
            console.log("Users seeded successfully.");

            // 2. Seed other data...
            console.log("Seeding customers, deals, etc...");
            await client.query(`
                INSERT INTO customers (id, name, industry, owner_id, notes) VALUES 
                ...
                -- Add other sample data here if needed
            `);
            ...
            await client.query(`
                INSERT INTO leads (id, name, company, status, source, owner_id, converted_customer_id) VALUES 
                    (1, 'John Miller', 'ACME Corp', 'Converted', 'Web Form', 1, 1), (2, 'Emily White', 'Synergy Group', 'Converted', 'Referral', 1, 2),
                    (3, 'Peter Chan', 'Global Innovations', 'Converted', 'Trade Show', 2, 3), (4, 'David Green', 'NextGen Enterprises', 'Lost', 'Cold Call', 2, NULL),
                    (5, 'Maria Garcia', 'Data Systems Co.', 'Contacted', 'Web Form', 3, NULL), (6, 'Bob Smith', 'Tech Solutions Inc.', 'New', 'Advertisement', 1, NULL),
                    (7, 'Dr. Eva Rostova', 'Quantum Solutions', 'Converted', 'Partner', 2, 4), (8, 'Frank Jaeger', 'Pioneer Logistics', 'Converted', 'Referral', 3, 5),
                    (9, 'Helen Chu', 'Apex Health', 'Converted', 'Web Form', 1, 6), (10, 'Leo Barnes', 'Starlight Media', 'New', 'Web Form', 1, NULL);
            `);
            ...
            await client.query(`
                INSERT INTO product_attachments (product_id, file_name, file_url) VALUES 
                    (1, 'datasheet_ws.pdf', '#'), 
                    (1, 'marketing_brief.pdf', '#'), 
                    (2, 'datasheet_laptop.pdf', '#');
                `)
            ...
            console.log("Other sample data seeded.");
